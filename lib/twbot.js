   ...
   if( self._plugins[modpath] === undefined ){
      ...
      if( mod.events ){
         mod.events.forEach(function(elm){
            self.logger.info(modpath + ": event '" + elm[0] + "'");
            self.on(elm[0], elm[1]);
         });
      }
      if( mod.maches ){
         mod.maches.forEach(function(elm){
            self.logger.info(modpath + ": match '" + elm[0] + "'");
...
TwBot.prototype.startUserStream = function(params){
   ...
   function _startUserStream(){
      ...
      stream.on('end', function(){
         self.logger.info('UserStream ends successfully. Reconnecting...');
   ...
   if( this._account == null ){
      this._client.getAccount(function(err, data){
         if( !err ){
            self.logger.info('UserStream (' + data.name + ') started ... ');
            self._account = data;
            _startUserStream();
         }else{
